...
-- Cat_{1,2}: C⁹ → Sym²(C⁹), 9×45  (optimised: no loop over coefficients)
buildCatMatrix = f -> (
...
-- Y_{(1,1)}: ∧²(C⁹)^* → (C⁹)^*, 9×36  (v10 optimised version)
buildYoungFlat11 = f -> (
...
-- 2.4: Apolarity projector onto span(det₃)
-- The apolarity inner product: ⟨f,g⟩ = Σ_α c_α(f)·c_α(g)·α!  where α! = Π αᵢ!
-- The projector π_{111}: f ↦ (⟨f,det₃⟩/⟨det₃,det₃⟩) · det₃
print "  2.4: Apolarity projector π_{(1,1,1)} onto span(det₃)"
print ""
...
print ("    ‖det₃‖²_apolar  = " | toString det3norm2)
print ("    ‖perm₃‖²_apolar = " | toString perm3norm2)
print ("    ⟨det₃,perm₃⟩    = " | toString det3xperm | "  (should be 0 by Schur's lemma)")
print ""

pi111 = f -> (apoInner(f, det3Poly) / det3norm2) * det3Poly
print ("    π_{111}(det₃) == det₃ ? " | toString(pi111(det3Poly) == det3Poly))
print ("    π_{111}(perm₃) == 0 ? " | toString(pi111(perm3Poly) == 0))
frand0 = random(3, R9)
print ("    π_{111}(random cubic) == 0 ? " | toString(pi111(frand0) == 0))
print "    [Expect yes: generic cubics have zero projection onto span(det₃)]"
print ""
...
-- 3.2: Y_{(1,1)}
print "  3.2: Y_{(1,1)} — 9×36 Young flattening [extended analysis]"
print ""
print "    Mathematical meaning:"
print "    Y_{(1,1)}(f)_{i,(p,q)} = coeff of y_i in ∂²f/∂y_p∂y_q  (p<q)"
print "    This is the 'antisymmetric Hessian map': ∧²V^* → V"
print ""
...
print ("    rank Y_{(1,1)}(det₃)  = " | toString rkY11det)
print ("    rank Y_{(1,1)}(perm₃) = " | toString rkY11perm)
print ("    rank Y_{(1,1)}(generic cubics, 5 samples): " | toString rkY11generic)
print ""

-- Gram matrix of Y_{(1,1)}(det₃): Y * Y^T (9×9)
gram11 = YDet3 * transpose YDet3
det11  = det gram11
print ("    det(Y_{(1,1)}(det₃)·Y_{(1,1)}(det₃)^T) = " | toString det11)
print ("    [Non-zero iff Y_{(1,1)}(det₃) has full rank 9]")
print ""
...
print ("    adj(Y)² - det(Y)·Y = 0 ? " | toString(adjSq_minus_detY == 0))
...
print "  4.4: GL₃×GL₃-equivariant invariants on orbit samples"
print ""
print "    Testing the apolarity with det₃: f ↦ ⟨f, det₃⟩"
print "    [This is a linear FUNCTIONAL on Sym³C⁹, equivariant for GL₃×GL₃]"
print ""

apoVals = apply(take(orbitPts, 20), f -> apoInner(f, det3Poly))
nzApo   = #select(apoVals, v -> v != 0)
print ("    ⟨f, det₃⟩ for 20 orbit samples: " | toString apoVals)
print ("    Non-zero: " | toString nzApo | "/20")
print "    [If most are non-zero: orbit is not confined to ⟨det₃⟩⊥ hyperplane]"
print ""

-- Testing Q_{111}(f) = ⟨f, det₃⟩² (equivariant degree-2 function)
print "    Q_{111}(f) = ⟨f, det₃⟩² on orbit samples:"
...
print ("    → Q_{111} ∉ I(Ō_{det₃}) (does not vanish on orbit)")
print ""
...
-- rv(f) = (rank Cat_{1,2}(f), rank Y_{(1,1)}(f), rank Hess(f)(y₀))
--
-- Strata:
...
-- Reference points
print "  5.1: Reference rank vectors rv = (Cat_{1,2}, Y_{(1,1)}, Hess(y₀))"
print ""
...
print "    For f ∈ O_{det₃}: is rank(Y_{(1,1)}(f)) constrained?"
print ""
...
if maxOrb < maxGen then (
    ...
    print ("    (" | toString(maxOrb+1) | ")×(" | toString(maxOrb+1)
           | ") minors of Y_{(1,1)}(f) belong to I(Ō_{det₃})!")
) else (
...
print ("    Verification: adj(Y)² = det₃·Y ? " | toString(adjSq_minus_detY == 0))
print ""

-- 7.2: The "Pfaffian" degree-3 equation candidate
print "  7.2: Degree-3 Pfaffian-type equation candidate"
print ""
print "    For f ∈ Sym³C⁹, define the 'curvature matrix':"
print "    K(f) = Y_{(1,1)}(f) · Y_{(1,1)}(f)^T  (9×9 symmetric matrix over kk^9)"
print "    evaluated at a fixed y₀. This is a 9×9 matrix depending on f."
print ""
print "    Conjecture: for f ∈ Ō_{det₃},"
print "    rank K(f)(y₀) ≤ r for a specific r < 9."
print "    This would give (r+1)×(r+1) minors of K as equations."
print ""

K_det3  = evalMatAt(YDet3 * transpose YDet3, y0fixed)  -- Y·Y^T evaluated
...
print ("    rank K(det₃)(y₀)  = " | toString rank K_det3)
print ("    rank K(perm₃)(y₀) = " | toString rank K_perm)
print ("    rank K(f)(y₀) for 10 orbit samples:   " | toString Korb)
print ("    rank K(f)(y₀) for 10 generic cubics:   " | toString Kgen)
print ""

if max Korb < max Kgen then (
    print "    *** DEGREE-3 PFAFFIAN OBSTRUCTION ***"
    print ("    rank K(f)(y₀) ≤ " | toString max Korb | " for f ∈ O_{det₃}")
    print ("    vs generic rank " | toString max Kgen)
    print ("    → Pfaffian-type equations exist in I(Ō_{det₃})!")
)
print ""
...
print ""
print "  Polynomial        | Cat_{1,2} | Y_{(1,1)} | Hess(y₀) | ‖apolar det₃‖"
print "  ─────────────────────────────────────────────────────────────────────"
...
print ""
print ("    dim O_{det₃} = " | toString dimOrbit | "  [= rank of tangent matrix]")
...
print ("    ‖det₃‖²_apolar = " | toString det3norm2)
print ("    ⟨det₃, perm₃⟩_apolar = " | toString det3xperm | " = 0  [Schur orthogonality]")
print ""
