...
--       artefacts).
--   (e) Boucle multi-prime sur primeModList (par defaut 3 premiers :
--       32003, 7919, 65537). Reste tague MODULAR (minorant, ne prouve
--       pas sur C). Objectif : detecter une chute de rang propre a un
--       prime specifique.
--
...
CONFIG#"seed"             = 42
-- 3 premiers : un large (32003), un Mersenne-like medium (7919), un Fermat-like (65537).
-- Objet du multi-prime : detecter une chute de rang due a un prime specifique.
-- Reste de statut MODULAR ; ne change pas le statut epistemique global.
CONFIG#"primeModList"     = {32003, 7919, 65537}
CONFIG#"runQQ"            = true
...
-- ================================================================
-- §11 — RANGS MODULAIRES (multi-prime, primeModList)  [MOD v19]
-- ================================================================
...
-- §13-bis : variables top-level pour transmettre le verdict observe a la synthese
-- (voir §15 final). Initialisees a "NOT_RUN" si runSampling=false.
obsDeltaGL9Det3Status   = "NOT_RUN"
obsDeltaGL9Perm3Status  = "NOT_RUN"
obsDeltaGL9Det3Tally    = ""
obsDeltaGL9Perm3Tally   = ""
obsDeltaGL9NSamples      = 0

if CONFIG#"runSampling" then (
    bigsep();
    print "Section 13-bis - delta sous GL9 (observe, NON certifie)";
    bigsep();
    nGL9 := 10;
    obsDeltaGL9NSamples = nGL9;
    ...
    valsPerm3 := apply(nGL9, idx -> (
        g := genGL9();
        fG := actionGL9onPoly(perm3K, g);
        CfG := buildCatMatrix fG;
        rank (piMinusMat * CfG)
    ));
    obsDeltaGL9Det3Tally  = toString tally valsDet3;
    obsDeltaGL9Perm3Tally = toString tally valsPerm3;
    obsDeltaGL9Det3Status  = if #unique valsDet3  > 1 then "VARIABLE" else "CONSTANT";
    obsDeltaGL9Perm3Status = if #unique valsPerm3 > 1 then "VARIABLE" else "CONSTANT";
    tagPrint("OBS-N", "delta(g.det3)  sur " | toString nGL9 | " tirages GL9 : " | obsDeltaGL9Det3Tally);
    tagPrint("OBS-N", "delta(g.perm3) sur " | toString nGL9 | " tirages GL9 : " | obsDeltaGL9Perm3Tally);
    -- Note epistemique : delta n'est PAS un invariant GL9 (Lemme structurel).
    -- L'observation ci-dessous decrit ce que ce run a vu, pas une propriete generale.
    if obsDeltaGL9Det3Status === "VARIABLE" then (
        tagPrint("NOTE", "delta(g.det3) varie -- coherent avec la non-invariance GL9 (mais ne la prouve pas)")
    ) else (
        tagPrint("NOTE", "delta(g.det3) constant a cette resolution -- N'IMPLIQUE PAS invariance GL9")
    );
    if obsDeltaGL9Perm3Status === "VARIABLE" then (
        tagPrint("NOTE", "delta(g.perm3) varie -- coherent avec la non-invariance GL9 (mais ne la prouve pas)")
    ) else (
        tagPrint("NOTE", "delta(g.perm3) constant a cette resolution -- N'IMPLIQUE PAS invariance GL9")
    )
)
...
-- gl9 delta sampling observations (semantique observee, pas certifiee)
jsonOut << "  \"gl9_delta_observations\": {" << endl
jsonOut << "    \"n_samples\": " << toString obsDeltaGL9NSamples << "," << endl
jsonOut << "    \"det3_status\": \""  << escapeJson obsDeltaGL9Det3Status  << "\"," << endl
jsonOut << "    \"det3_tally\": \""   << escapeJson obsDeltaGL9Det3Tally   << "\"," << endl
jsonOut << "    \"perm3_status\": \"" << escapeJson obsDeltaGL9Perm3Status << "\"," << endl
jsonOut << "    \"perm3_tally\": \""  << escapeJson obsDeltaGL9Perm3Tally  << "\"" << endl
jsonOut << "  }," << endl
...
tagPrint("OBS-N", "delta constant sur orbite GL3xGL3 (lemmes 1, 3 ; observe)")
-- Synthese fidele a ce que §13-bis a observe ce run-ci. Pas de claim figee.
if obsDeltaGL9Det3Status === "NOT_RUN" then (
    tagPrint("NOTE", "delta sous GL9 : non echantillonne (CONFIG runSampling = false)")
) else (
    tagPrint("OBS-N",
        "delta(g.det3) GL9 (" | toString obsDeltaGL9NSamples | " tirages) : "
        | obsDeltaGL9Det3Status | " -- " | obsDeltaGL9Det3Tally);
    tagPrint("OBS-N",
        "delta(g.perm3) GL9 (" | toString obsDeltaGL9NSamples | " tirages) : "
        | obsDeltaGL9Perm3Status | " -- " | obsDeltaGL9Perm3Tally);
    tagPrint("NOTE", "delta n'est PAS un invariant GL9 (structurel §13) ; constance ou variabilite observee = artefact d'echantillonnage, pas une preuve")
)
...
-- ================================================================
-- §17 — VÉRIFICATION INTÉGRÉE (replay in-process, partiel)  [NEW v19]
-- ================================================================
--
-- Verifie apres-coup, dans un anneau FRAIS (R9V = QQ[z_0..z_8])
-- totalement disjoint des anneaux de travail R9 et Sym3R9, deux
-- proprietes algebriques critiques :
--   (a) dim Sing V(det3) = 5 et dim Sing V(perm3) = 3,
--   (b) identite cofacteurs : adj(Y) * Y = det(Y) * I_3.
--
-- Verifie egalement la coherence statut/tally du bloc OBS-N GL9 et
-- le maintien de NIV-2 dir.2 a OPEN.
--
-- C'est un sous-ensemble strict de srmt_v19_verify.m2 (qui relit
-- AUSSI le certificat JSON sans acces au pipeline). Les deux voies
-- restent disponibles. En cas d'echec : error() => exit non zero,
-- run = NON CONFORME.

bigsep()
print "Section 17 - Verification integree (replay in-process, partiel)"
bigsep()

verifyPassed = true
verifyFailures = {}

recordVerify = (label, ok, detail) -> (
    if ok then (
        tagPrint("INFO", "VERIFY OK : " | label)
    ) else (
        verifyPassed = false;
        verifyFailures = append(verifyFailures, label | " -- " | detail);
        tagPrint("WARN", "VERIFY FAIL : " | label | " -- " | detail)
    )
);

-- Anneau frais, totalement disjoint de R9 et Sym3R9
R9V = QQ[z_0..z_8, MonomialOrder => GRevLex]
MV = genericMatrix(R9V, 3, 3)
det3V = det MV
-- permanent 3x3 explicite (pas de native M2)
perm3V = (
    MV_(0,0)*MV_(1,1)*MV_(2,2) + MV_(0,0)*MV_(1,2)*MV_(2,1)
  + MV_(0,1)*MV_(1,0)*MV_(2,2) + MV_(0,1)*MV_(1,2)*MV_(2,0)
  + MV_(0,2)*MV_(1,0)*MV_(2,1) + MV_(0,2)*MV_(1,1)*MV_(2,0)
)

-- (a) dim Sing V(f) = dim de l'ideal singulier (f + jacobien)
singDet3V  = ideal det3V  + ideal jacobian matrix{{det3V}}
singPerm3V = ideal perm3V + ideal jacobian matrix{{perm3V}}
dimSingDet3V  = dim singDet3V
dimSingPerm3V = dim singPerm3V
recordVerify("dim_Sing_det3_QQ_recomputed",
    dimSingDet3V  == 5,
    "recompute=" | toString dimSingDet3V  | " expected=5");
recordVerify("dim_Sing_perm3_QQ_recomputed",
    dimSingPerm3V == 3,
    "recompute=" | toString dimSingPerm3V | " expected=3");

-- (b) identite cofacteurs : adj(Y) * Y = det(Y) * I sur QQ[z]
adjV = matrix table(3, 3,
    (i, j) -> (-1)^(i+j) * det submatrix'(MV, {j}, {i}))
idCheckV = (adjV * MV) - det3V * id_(R9V^3)
recordVerify("adj_identity_QQ_recomputed",
    idCheckV == 0,
    "adj(Y)*Y - det(Y)*I != 0 dans QQ[z]");

-- (c) coherence interne gl9_delta_observations (status vs tally)
checkTallyCoherence = (status, tally) -> (
    if status === "NOT_RUN" or tally === "" then true
    else (
        nArrows := #(select(characters tally, c -> c === ">"));
        if status === "CONSTANT"  and nArrows >  1 then false
        else if status === "VARIABLE" and nArrows <= 1 then false
        else true
    )
);
recordVerify("gl9_obs_det3_internal_coherent",
    checkTallyCoherence(obsDeltaGL9Det3Status,  obsDeltaGL9Det3Tally),
    "status=" | obsDeltaGL9Det3Status  | " tally=" | obsDeltaGL9Det3Tally);
recordVerify("gl9_obs_perm3_internal_coherent",
    checkTallyCoherence(obsDeltaGL9Perm3Status, obsDeltaGL9Perm3Tally),
    "status=" | obsDeltaGL9Perm3Status | " tally=" | obsDeltaGL9Perm3Tally);

-- (d) verrou semantique : NIV-2 dir.2 doit rester OPEN (conjecture GCT centrale)
recordVerify("NIV2_dir2_remains_OPEN",
    nonDegenDet3NotInGL9perm3 === "OPEN",
    "got=" | nonDegenDet3NotInGL9perm3 |
    " -- conjecture GCT centrale ne doit PAS etre marquee PROVED");
...
print ""
if verifyPassed then (
    tagPrint("INFO", "§17 VERIFICATION INTEGREE : PASS (replay in-process conforme)")
) else (
    tagPrint("WARN", "§17 VERIFICATION INTEGREE : FAIL (" |
        toString (#verifyFailures) | " echec(s))");
    scan(verifyFailures, msg -> tagPrint("WARN", "  - " | msg));
    error("[VERIFY] Replay in-process divergent du manifeste -- run NON CONFORME")
);

-- ================================================================
-- §18 — VÉRIFICATION JSON (replay tiers, fusionne)  [NEW v19]
-- ================================================================
--
-- Reprend la voie du fichier autonome srmt_v19_verify.m2 : relecture
-- brute du certificat JSON tout juste ecrit, et comparaison des
-- valeurs "observed" aux valeurs attendues du manifeste. Cette voie
-- est INDEPENDANTE de §15 (recalcul interne) et de §17 (replay
-- in-process en anneau frais) : elle ne consulte que le JSON,
-- comme le ferait un auditeur externe.
--
-- Trois canaux de verification independants se confirment alors
-- mutuellement :
--   §15  : calcul source dans R9 / Sym3R9
--   §17  : recalcul in-process en anneau frais R9V
--   §18  : relecture du JSON, comparaison aux EXPECTED figes
--
-- En cas de divergence : error() => exit non zero.

bigsep()
print "Section 18 - Verification JSON (replay tiers, fusionne)"
bigsep()
...
-- Normalisation : retire espaces, et harmonise {a,b,c} <-> [a,b,c]
normStr = s -> (
    t := concatenate select(characters s, c -> c =!= " " and c =!= "\t");
    t = replace("\\{", "[", t);
    t = replace("\\}", "]", t);
    t
);

-- Extraction d'un champ JSON niveau zero (delimiteurs ',' ou '}').
extractJsonField = (line, fname) -> (
    needle := "\"" | fname | "\":";
    pos := regex(needle, line);
    if pos === null then ""
    else (
        startIdx := pos#0#0 + pos#0#1;
        nLen := #line;
        while startIdx < nLen and
            (line#startIdx === " " or line#startIdx === "\t") do
            startIdx = startIdx + 1;
        depthBracket := 0;
        inString := false;
        ii := startIdx;
        done := false;
        while ii < nLen and not done do (
            c := line#ii;
            if inString then (
                if c === "\"" then inString = false;
                ii = ii + 1
            ) else (
                if c === "\"" then (inString = true; ii = ii + 1)
                else if c === "[" then
                    (depthBracket = depthBracket + 1; ii = ii + 1)
                else if c === "]" then
                    (depthBracket = depthBracket - 1; ii = ii + 1)
                else if depthBracket == 0 and
                    (c === "," or c === "}") then done = true
                else ii = ii + 1
            )
        );
        jj := ii;
        while jj > startIdx and
            (line#(jj-1) === " " or line#(jj-1) === "\t") do
            jj = jj - 1;
        raw := substring(line, startIdx, jj - startIdx);
...
if not certPresent or #certText == 0 then (
    tagPrint("WARN", "§18 : certificat introuvable : " | jsonFile);
    error("[VERIFY-JSON] Certificat absent ou vide")
);
...
scan(certLines, ln -> (
    if match("\"key\":", ln) then (
        keyStr := extractJsonField(ln, "key");
        if keyStr =!= "" then
            certObserved#keyStr = extractJsonField(ln, "observed")
    )
));

tagPrint("INFO",
    "§18 : " | toString (#keys certObserved) | " claim(s) lu(es) du JSON");
...
scan(EXPECTED_JSON, pair -> (
    k := pair#0;
    expS := normStr pair#1;
    if not certObserved#?k then (
        nJsonMissing = nJsonMissing + 1;
        jsonFailures = append(jsonFailures, k | " : MISSING dans le JSON");
        tagPrint("WARN", "§18 MISSING : " | k)
    ) else (
        obsS := normStr certObserved#k;
        if obsS === expS then (
            nJsonOk = nJsonOk + 1
        ) else (
            nJsonMismatch = nJsonMismatch + 1;
            jsonFailures = append(jsonFailures,
                k | " : observed=" | obsS | " expected=" | expS);
            tagPrint("WARN",
                "§18 MISMATCH : " | k |
                " observed=" | obsS | " expected=" | expS)
        )
    )
));

tagPrint("INFO",
    "§18 : OK="        | toString nJsonOk |
    " MISMATCH="        | toString nJsonMismatch |
    " MISSING="         | toString nJsonMissing);

if nJsonMismatch > 0 or nJsonMissing > 0 then (
    scan(jsonFailures, msg -> tagPrint("WARN", "  - " | msg));
    error("[VERIFY-JSON] Certificat divergent du manifeste " |
          "-- run NON CONFORME")
);

tagPrint("INFO",
    "§18 VERIFICATION JSON : PASS (relecture certificat conforme)")

logStep("pipeline_end", "COMPLETE",
    "SRMT v19 finished, manifest enforced, " |
    "verify in-process passed, verify JSON passed")
exit 0
